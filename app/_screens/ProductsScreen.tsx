...
//     setForm(blankProductForm(defaultCategoryId));
//     setImageData(undefined);
//     setImagePreview(null);
//     if (imageInputRef.current) imageInputRef.current.value = "";
//     setFormError("");
...
//         <label>สินค้า<select value={stockForm.productId} onChange={(event) => setStockForm((current) => ({ ...current, productId: event.target.value }))} required>{products.map((product) => <option value={product.id} key={product.id}>{product.name} (คงเหลือ {numberText(product.stockQuantity)} {product.unit})</option>)}</select></label>
//         <div className="product-form-grid">
//           <label>จำนวนเติมเข้า<input type="number" min="0.001" step="0.001" value={stockForm.quantity} onChange={(event) => setStockForm((current) => ({ ...current, quantity: event.target.value }))} placeholder="0" required /></label>
//           <label>ต้นทุนต่อหน่วย (บาท)<input type="number" min="0" step="0.01" value={stockForm.unitCost} onChange={(event) => setStockForm((current) => ({ ...current, unitCost: event.target.value }))} placeholder="0.00" /></label>
//           <label className="wide">Supplier<select value={stockForm.supplierId} onChange={(event) => setStockForm((current) => ({ ...current, supplierId: event.target.value }))}><option value="">ใช้ผู้จำหน่ายทั่วไป</option>{suppliers.map((supplier) => <option value={supplier.id} key={supplier.id}>{supplier.name}</option>)}</select></label>
//           <label className="wide">หมายเหตุ<input value={stockForm.note} onChange={(event) => setStockForm((current) => ({ ...current, note: event.target.value }))} placeholder="เช่น เลขที่ใบรับสินค้า" /></label>
//         </div>
//         <div className="product-form-actions"><button type="button" onClick={closeStockForm}>ยกเลิก</button><button className="primary-button" type="submit" disabled={stockSaving}>{stockSaving ? "กำลังบันทึก..." : "บันทึกเติมสต็อก"}</button></div>
...
//             <label>SKU<input value={form.sku} onChange={(event) => updateField("sku", event.target.value)} placeholder="เช่น ING-SUGAR-01" required /></label>
//             <label>ชื่อสินค้า<input value={form.name} onChange={(event) => updateField("name", event.target.value)} placeholder="ชื่อสินค้า" required /></label>
//             <label>หมวดหมู่<select value={form.categoryId} onChange={(event) => updateField("categoryId", event.target.value)} required>{categories.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select></label>
//             <label>Supplier<select value={form.supplierId} onChange={(event) => updateField("supplierId", event.target.value)}><option value="">ไม่ระบุ</option>{suppliers.map((supplier) => <option value={supplier.id} key={supplier.id}>{supplier.name}</option>)}</select></label>
//             <label>หน่วยสินค้า<input value={form.unit} onChange={(event) => updateField("unit", event.target.value)} placeholder="เช่น ชิ้น, กิโลกรัม" required /></label>
//             <label>ราคาขาย (บาท)<input type="number" min="0" step="0.01" value={form.price} onChange={(event) => updateField("price", event.target.value)} placeholder="0.00" required /></label>
//             <label>จำนวนเริ่มต้น<input type="number" min="0" step="0.001" value={form.stockQuantity} onChange={(event) => updateField("stockQuantity", event.target.value)} placeholder="0" required /></label>
//             <label>จุดแจ้งเตือนสต็อก<input type="number" min="0" step="0.001" value={form.lowStockThreshold} onChange={(event) => updateField("lowStockThreshold", event.target.value)} placeholder="0" required /></label>
//             <label className="wide">รูปสินค้า<input ref={imageInputRef} type="file" accept="image/png,image/jpeg,image/webp" onChange={chooseImage} /><small className="image-help">PNG, JPG หรือ WEBP ขนาดไม่เกิน 2 MB</small></label>
//             {imagePreview && <div className="image-preview wide"><img src={imagePreview} alt="ตัวอย่างรูปสินค้า" /><button type="button" onClick={() => { setImageData(editing ? null : undefined); setImagePreview(null); }}>ล้างรูป</button></div>}
//           </div>
//         </div>
...
type Product = {
  ...
  stockQuantity: number;
  lowStockThreshold: number;
  expiryDate: string | null; // 🟢 วันหมดอายุ
  imageUrl: string | null;
  isActive: number;
};
...
type ProductForm = {
  ...
  supplierId: string;
  unit: string;
  costPrice: string; // 🟢 เพิ่มราคาต้นทุน
  price: string;     // ราคาขาย
  stockQuantity: string;
  lowStockThreshold: string;
  expiryDate: string; // 🟢 เพิ่มวันที่หมดอายุ
};

type SupplierForm = { name: string; phone: string; address: string };
type StockForm = { productId: string; supplierId: string; quantity: string; unitCost: string; note: string };

const DEFAULT_UNITS = ["ชิ้น", "กล่อง", "ขวด", "ถุง", "แพ็ค", "กิโลกรัม", "กรัม", "ลิตร", "มิลลิลิตร", "ชุด", "แก้ว", "กระป๋อง"];

function blankProductForm(categoryId = "", defaultUnit = "ชิ้น"): ProductForm {
  return { 
    sku: "", 
    name: "", 
    categoryId, 
    supplierId: "", 
    unit: defaultUnit, 
    costPrice: "", // 🟢 ค่าเริ่มต้นต้นทุน
    price: "", 
    stockQuantity: "", 
    lowStockThreshold: "",
    expiryDate: "" // 🟢 ค่าเริ่มต้นวันหมดอายุ
  };
}
...
export default function ProductsScreen() {
  ...
  const unitOptions = useMemo(() => {
    const existingProductUnits = products.map((p) => p.unit).filter(Boolean);
    const combined = Array.from(new Set([...DEFAULT_UNITS, ...existingProductUnits, ...customUnits]));
    if (form.unit && !combined.includes(form.unit)) {
      combined.push(form.unit);
    }
    return combined;
  }, [products, customUnits, form.unit]);
  ...
  const openCreate = () => {
    ...
    setEditing(null);
    const defaultCategoryId = categories.length > 0 ? String(categories[0].id) : "";
    setForm(blankProductForm(defaultCategoryId, unitOptions[0] || "ชิ้น"));
    setImageData(undefined);
    setImagePreview(null);
    if (imageInputRef.current) imageInputRef.current.value = "";
    setFormError("");
  ...
  const saveProduct = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const payload = {
        ...
        supplierId: form.supplierId ? Number(form.supplierId) : null,
        unit: form.unit.trim(),
        costPrice: Number(form.costPrice || 0), // 🟢 ส่งราคาต้นทุน
        price: Number(form.price),
        stockQuantity: Number(form.stockQuantity),
        lowStockThreshold: Number(form.lowStockThreshold),
        expiryDate: form.expiryDate ? form.expiryDate : null, // 🟢 ส่งวันที่หมดอายุ
        imageData,
      };
      if (!payload.sku || !payload.name || !payload.categoryId || !payload.unit) {
        throw new Error("กรุณากรอกข้อมูลสินค้าให้ครบถ้วน");
  ...
  return <AdminShell active="products">
    ...
    {/* Modal Form เพิ่มสินค้าใหม่ / แก้ไขสินค้า */}
    {formOpen ? <div className="modal-backdrop" onMouseDown={closeForm}>
      <form className="modal product-form-modal" onSubmit={saveProduct} onMouseDown={(event) => event.stopPropagation()}>
        <button type="button" className="modal-close" onClick={closeForm} aria-label="ปิด"><X /></button>
        <h2>{editing ? "แก้ไขสินค้า" : "เพิ่มสินค้าใหม่"}</h2>
        ...
        <div className="product-form-scroll">
          <div className="product-form-grid">
            ...
            <label>หน่วยสินค้า
              <select 
                ...
                onChange={(event) => {
                  ...
                  if (val === "__ADD_NEW__") {
                    const custom = window.prompt("กรอกหน่วยสินค้าใหม่ (เช่น จาน, ถ้วย, โหล):");
                    if (custom && custom.trim()) {
                      const trimmed = custom.trim();
                      setCustomUnits((prev) => [...prev, trimmed]);
                      updateField("unit", trimmed);
                    }
                  } else {
              ...
              >
                ...
                <option value="__ADD_NEW__">+ เพิ่มหน่วยใหม่...</option>
              </select>
            </label>

            {/* 🟢 เพิ่ม ราคาต้นทุน (บาท) */}
            <label>ราคาต้นทุน (บาท)
              <input type="number" min="0" step="0.01" value={form.costPrice} onChange={(event) => updateField("costPrice", event.target.value)} placeholder="0.00" />
            </label>
            ...
            {/* 🟢 เพิ่ม วันที่หมดอายุ */}
            <label>วันที่หมดอายุ
              <input type="date" value={form.expiryDate} onChange={(event) => updateField("expiryDate", event.target.value)} />
            </label>
